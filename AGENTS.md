...
- `uniapp` 新增或重构业务代码时，默认使用 TypeScript。
- `uniapp` 的业务接口、业务类型定义、页面脚本优先使用 `.ts` 与 `<script lang="ts">`，不要继续新增松散的业务 `.js` 文件。
- 前端涉及接口返回、筛选条件、页面状态、组件入参时，必须补充明确的数据结构约束，不允许依赖隐式 `any` 或无约束对象长期扩散。
...
用于执行更新的增量 SQL 文件按仓库既有规则默认不纳入 git 管理；只同步维护 `backend/src/main/resources/db/schema.sql` 作为结构基准。
