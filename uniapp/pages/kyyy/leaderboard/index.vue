<!--
@file KyyyLeaderboardPage
@project pipker-do
@module 考研英语 / 小程序排行榜
@description 展示英语综合学习排行榜，聚合背词与阅读成绩并突出个人当前名次。
@logic 1. 支持日榜、周榜、总榜切换；2. 展示我的排名焦点卡、TOP3 与完整榜单；3. 处理加载、刷新、失败重试和规则说明弹层。
@dependencies API: @/pages/kyyy/api/leaderboard, View: @/pages/kyyy/leaderboard/view, Shared: @/shared/session/session
@index_tags 考研英语, 排行榜, 综合学习榜, 小程序页面
@author holic512
-->
<template>
	<view class="leaderboard-page" @tap="handlePageTap">
		...
		<view class="leaderboard-page__my-card">
			<view class="leaderboard-page__my-head">
				<view class="leaderboard-page__my-title">
					<uni-icons type="staff-filled" size="18" color="#486152" />
			...
			<view v-if="dashboard.myRecord" class="leaderboard-page__my-main">
				...
				<view class="leaderboard-page__my-metrics">
					<view class="leaderboard-page__metric-pill">
						...
						<text class="leaderboard-page__metric-text">学习量 {{ dashboard.myRecord.studyCount }}</text>
					</view>
					<view class="leaderboard-page__metric-pill">
						<uni-icons type="checkbox-filled" size="14" color="#5e8d69" />
						<text class="leaderboard-page__metric-text">有效掌握/答对 {{ dashboard.myRecord.correctCount }}</text>
					</view>
					<view class="leaderboard-page__metric-pill">
			...
			<view v-else class="leaderboard-page__my-empty">
				...
				<text class="leaderboard-page__my-empty-desc">先背词或做阅读就会进入榜单，焦点卡会及时同步你的阶段位置。</text>
				<button class="leaderboard-page__my-empty-action" @tap="goPractice">去学习冲榜</button>
			</view>
		</view>
		...
		<view v-if="loading && !loadedOnce" class="leaderboard-page__state-card">
			...
			<text class="leaderboard-page__state-desc">会按综合学习量、正确率与活跃时间生成榜单。</text>
		</view>
		...
		<template v-else>
			<view v-if="topThreeRecords.length" class="leaderboard-page__top3">
				<view class="leaderboard-page__section-head">
					...
					<text class="leaderboard-page__section-meta">综合背词与阅读数据实时聚合</text>
				</view>
				<view class="leaderboard-page__top3-list">
					...
					>
						...
						<text class="leaderboard-page__top3-score">{{ item.studyCount }} 次 · {{ formatAccuracyRate(item.accuracyRate) }}</text>
					</view>
				</view>
			</view>
			...
			<view v-if="dashboard.records.length" class="leaderboard-page__full-list">
				<view class="leaderboard-page__section-head">
					...
					<text class="leaderboard-page__section-meta">学习量、有效掌握/答对数、正确率全部公开</text>
				</view>
				<view
					...
					<view class="leaderboard-page__row-main">
						...
						<view class="leaderboard-page__row-metrics">
							<view class="leaderboard-page__metric-pill">
								...
								<text class="leaderboard-page__metric-text">学习量 {{ item.studyCount }}</text>
							</view>
							<view class="leaderboard-page__metric-pill">
								<uni-icons type="checkbox-filled" size="13" color="#5f8a6b" />
								<text class="leaderboard-page__metric-text">有效掌握/答对 {{ item.correctCount }}</text>
							</view>
							<view class="leaderboard-page__metric-pill">
			...
			<view v-else class="leaderboard-page__state-card">
				...
				<text class="leaderboard-page__state-desc">先背词或做阅读后会自动进入该区间排行榜，并在上方焦点卡同步你的名次。</text>
				<button class="leaderboard-page__retry-btn" @tap="goPractice">去学习</button>
			</view>
		</template>
	</view>
</template>
...
import { getLeaderboard } from '@/pages/kyyy/api/leaderboard'
import type {
...
} from '@/pages/kyyy/leaderboard/view'
...
interface LeaderboardPageState {
	...
	currentScope: KyyyLeaderboardScope
	requestToken: number
	dashboard: KyyyLeaderboardDashboardState
...
export default defineComponent({
		...
		return {
			...
			scopeTabs: [
				...
				},
				{
					...
					icon: 'medal-filled',
					iconColor: '#6b5f2a',
					iconShellBackground: '#fbf4df'
				}
			]
		}
	},
	computed: {
		topThreeRecords(): KyyyLeaderboardRecordState[] {
	...
	methods: {
		formatAccuracyRate,
		formatLeaderboardTime,
		scopeLabel(scope: KyyyLeaderboardScope): string {
			if (scope === 'weekly') {
				return '周榜'
			}
			if (scope === 'total') {
		...
		async handleScopeChange(scope: KyyyLeaderboardScope): Promise<void> {
		...
		goPractice(): void {
			this.showRulePopover = false
			uni.reLaunch({
				url: '/pages/kyyy/practice/index?mode=study',
				fail: () => {
					uni.showToast({
						title: '跳转学习失败',
						icon: 'none'
...
.leaderboard-page {
	...
	background: linear-gradient(180deg, #f7faf8 0%, #eef4f0 52%, #e8efea 100%);
}
...
.leaderboard-page__scope-tab.is-active {
	background: #edf6f0;
	box-shadow:
		0 14rpx 24rpx rgba(72, 97, 82, 0.1),
		inset 0 0 0 1rpx #9fc0aa;
}
...
.leaderboard-page__my-range {
	...
	box-shadow: inset 0 0 0 1rpx #c6d8cb;
}
...
.leaderboard-page__my-empty-action,
.leaderboard-page__retry-btn {
	background: linear-gradient(135deg, #476455 0%, #6d8778 100%);
	color: #ffffff;
	box-shadow: 0 12rpx 22rpx rgba(71, 100, 85, 0.18);
}
...
.leaderboard-page__me-badge {
	...
	padding: 0 12rpx;
	border-radius: 999rpx;
	background: linear-gradient(135deg, #dcecdc 0%, #edf8ef 100%);
	color: #406049;
	font-size: 20rpx;
	font-weight: 700;
}
...
.leaderboard-page__avatar--fallback {
	...
	align-items: center;
	justify-content: center;
	background: linear-gradient(135deg, #7f9a88 0%, #a7b7ae 100%);
}
